...
class Actor(nn.Module):
    def __init__(self, state_dim, action_dim, hidden_width, upper_limits, lower_limits):
        super(Actor, self).__init__()
        self.lower_limits = torch.Tensor(lower_limits)
        self.upper_limits = torch.Tensor(upper_limits)
    ...
    def forward(self, s):
        ...
        a = torch.tanh(self.l3(s)) + 1  # [0,2]
        param_range = self.upper_limits - self.lower_limits
        a = (a * param_range)/2 + self.lower_limits
        return a
...
class DDPG(object):
    def __init__(self, state_dim, action_dim, upper_limits, lower_limits):
        ...
        self.actor = Actor(state_dim, action_dim, self.hidden_width, upper_limits, lower_limits)
...
if __name__ == '__main__':
    ...
    number = 1
    env = DummyContainerEnvCont(num_features=NUM_STATE_FEATURES)
    env_evaluate = DummyContainerEnvCont(num_features=NUM_STATE_FEATURES)
    # Set random seed
    seed = 0
    env.seed(seed)
    ...
    # Build a tensorboard
    writer = SummaryWriter(log_dir='runs/DDPG/DDPG_env_{}_number_{}_seed_{}'.format("Containers", number, seed))
    ...
    random_steps = 25e2  # Take the random actions in the beginning for the better exploration
    ...
    stats = EpisodeStats(
        episode_lengths=np.zeros(600),
        episode_rewards=np.zeros(600))   
    ep = 0
    while total_steps < max_train_steps:
        ...
        while not done:
            ...
            if total_steps < random_steps:  # Take the random actions in the beginning for the better exploration
                ...
                print(a)
                a = (a + np.random.normal(0, noise_std(epsilon,max_action), size=action_dim)).clip(-max_action, max_action)
            s_, r, done, _ = env.step(a)
            ...
            if (total_steps + 1) % evaluate_freq == 0:
                ...
                stats.episode_rewards[evaluate_num-1] += evaluate_reward
                stats.episode_lengths[evaluate_num-1] = max_episode_steps
                evaluate_rewards.append(evaluate_reward)
                print("evaluate_num:{} \t evaluate_reward:{}".format(evaluate_num, evaluate_reward))
                writer.add_scalar('step_rewards_{}'.format("Containers"), evaluate_reward, global_step=total_steps)
                # Save the rewards
                if evaluate_num % 10 == 0:
                    np.save('./data_train/DDPG_env_{}_number_{}_seed_{}.npy'.format("Containers", number, seed), np.array(evaluate_rewards))
